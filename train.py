...
def train(hyp, opt, device, tb_writer=None):
    logger.info(colorstr('hyperparameters: ') + ', '.join(
        f'{k}={v}' for k, v in hyp.items()))
    save_dir, epochs, batch_size, total_batch_size, weights, rank, freeze = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, \
        opt.weights, opt.local_rank, opt.freeze
    ...
    if rank in [-1, 0]:
        ...
        run_id = torch.load(weights, map_location=device).get(
            'wandb_id') if weights.endswith('.pt') and os.path.isfile(
            weights) else None
    ...
    if rank in [-1, 0]:
        testloader = \
        create_dataloader(test_path, imgsz_test, batch_size * 2, gs, opt,
                          # testloader
    ...
    #                     isinstance(layer, nn.MultiheadAttention) for layer in
    #                     model.modules()))
    ...
    results = (
    0, 0, 0, 0, 0, 0, 0)  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
    ...
    for epoch in range(start_epoch, epochs):  # epoch
        ...
        if opt.image_weights:
            ...
            # if rank != -1:
            #     indices = (
            #         torch.tensor(dataset.indices) if rank == 0 else torch.zeros(
            #             dataset.n)).int()
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -----------
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ns = [math.ceil(x * sf / gs) * gs for x in imgs.shape[
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 10:
                    ...
                    wandb_logger.log({"Mosaics": [
                        wandb_logger.wandb.Image(str(x), caption=x.name) for x
                        in
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (
            last):  # speed, mAP tests
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='iseauto_data/iseauto.yaml',
                        help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='iseauto_data/hyp.scratch.p5.yaml',
    ...
    #             v = np.ones(ng)
    #             while all(
    #                     v == 1):  # mutate until a change occurs (prevent duplicates)
    #                 v = (g * (npr.random(ng) < mp) * npr.randn(
    #                     ng) * npr.random() * s + 1).clip(0.3, 3.0)
